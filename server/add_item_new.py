...
class addItemInterface:
    def __init__(self, wb_id, wm_id, sku, cof, category):
    ...
    def _get_values_new_item(self):
        ...
        return ([f'https://wmart.kz/prod/{self._get_mw_id}/',
                 ...
                 self._get_wb_id, self._get_mw_id, self._sku, self._cof, self._category])
    ...
    def _save_new_item(self, path):
        fm_add = fileManagerAdd(self._start_parser_new_item(path), self._values_wrapper[2], self._values_wrapper[3], self._values_wrapper[0], self._values_wrapper[1], self._values_wrapper[4], self._values_wrapper[5], self._values_wrapper[6])
        fm_add.add_item()
...
def add_item(wb_id, wm_id, sku, cof, category):
    path = 'server/chromedriver/chromedriver'
    new_item = addItemInterface(wb_id, wm_id, sku, cof, category)
