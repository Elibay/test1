...
class database:
    ...
    def __init__(self):
        ...
        self.cur.execute("PRAGMA table_info(wb_wm)")
        columns = [column[1] for column in self.cur.fetchall()]
        if 'ff_price' not in columns:
            self.cur.execute('ALTER TABLE wb_wm ADD COLUMN ff_price INTEGER DEFAULT 0')

        if 'logistics' not in columns:
            self.cur.execute('ALTER TABLE wb_wm ADD COLUMN logistics INTEGER DEFAULT 0')
...
class getItems(database):
    ...
    def get_items(self):
        items = self.cur.execute(
            'SELECT name, current_price, old_price, count, wb_id, wm_id, wb_link, wm_link, sku, cof, category, ff_price, logistics FROM wb_wm').fetchall()
        items_dict = {
            ...
            'ff_price': tuple(i[11] for i in items),
            'logistics': tuple(i[12] for i in items),
        }
        self.con.close()
        return items_dict
...
class addNewItem(database):
    ...
    def __init__(self, wm_id, wb_id, sku, cof, category, ff_price, logistics):
        super().__init__()
        self._wm_id = wm_id
        self._wb_id = wb_id
        self._sku = sku
    ...
    def add_item(self):
        ...
        wm_item_info.append(self.ff_price)
        wm_item_info.append(self.logistics)
        ff_and_logistics = self.ff_price + self.logistics

        wb_price = get_calculation_price(wm_item_info[1], self._cof, self._category, ff_and_logistics, get_rate())

        wm_item_info.append(wb_price)

        self.cur.execute(
            'INSERT INTO wb_wm(name, old_price, count, wb_id, wm_id, wb_link, wm_link, sku, cof, category, ff_price, logistics, current_price) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',
            tuple(wm_item_info))
        self.con.commit()
        self.con.close()
